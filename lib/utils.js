...
export function changeStateHelper(state, verb) {
  return {
    isProcessing: state === STATES.PENDING,
    isProcessed: state === STATES.SYNCED,
    isProcessingFailed: state === STATES.FAILED,
...
export function getParams(store, globalParams, params, raw, config = {}) {
  const { headersExtractor } = config;
  ...
  return {
    ...
    headers: {
      ...
      ...(headersExtractor ? headersExtractor(store) : {}),
    },
  };
}

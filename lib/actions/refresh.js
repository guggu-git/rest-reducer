...
import removeAllAction, { handleResponse as handleRemoveAllResponse } from './removeAll';
...
async function fetchAccessToken(options) {
  ...
  const { refreshTokenPayloadExtractor, refreshTokenACTION, refreshTokenURL } = config;
...
export default function refresh(options, action) {
  queue.push({ options, action });

import refreshToken, { canRefreshToken, isRefreshing, addToQueue } from './refresh'; // eslint-disable-line import/no-cycle
...
export async function handleResponse(response, options, replay = false) {
  const { ACTIONS, dispatch, query, config, getState } = options;
  ...
  } else if (response.status === 401 && canRefreshToken(config, getState()) && !replay) {
...
export default async function getAllAction(options, replay = false) {
  const {
    ACTIONS,
    dispatch,
    getState,
